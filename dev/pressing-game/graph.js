function Graph(graph_id)
{
	...
	this.toggleColor = function()
	{
		...
		{
			var toggleIndicator = d3.select("#toggleIndicator");
			var toggleIndicator1 = d3.select("#toggleIndicator1");
			var toggleIndicator2 = d3.select("#toggleIndicator2");
			if (self.new_vertex_color === "dark")
			{
				self.new_vertex_color = "light";
				toggleIndicator.attr("fill", "#ffffff")
				toggleIndicator1.attr("fill", "#ffffff")
				toggleIndicator2.attr("fill", "#ffffff")
				self.change_color_button_text_color(self.new_vertex_color);
			}
			else
			{
				self.new_vertex_color = "dark";
				toggleIndicator.attr("fill", "#000000")
				toggleIndicator1.attr("fill", "#000000")
				toggleIndicator2.attr("fill", "#000000")
				self.change_color_button_text_color(self.new_vertex_color);
			}
		}
		else
	...
	{
		var textColor = d3.selectAll("#color_text");
		var color = d3.selectAll("#color");

		if (color === "dark")
		{ 
			textColor.attr("fill", "#ffffff");
			color.attr("fill", "#000000");
		}
		else
		{
	...
	this.clear = function()
	{
		...
		*/
		...
		self.disableSequenceButton();
		self.disableMatrixButton();
	...
	this.getCoordinate = function(num){
	...
	this.random = function()
	{
		...
		var all_coordinates = self.getCoordinate(max_random_vertices);	
	...
	this.draw_matrix = function(target, x, initial_y)
	{
		...
		var x_loc = 190 + 15 * vertexCount.toString().length;
		...
		for (var row = 1; row <= m.length; row++) {
			...
			label_spaces = spaceArray[spaces_needed - row_digits.length];
			printed_row = spaceArray[row_digits.length] + label_spaces +
				JSON.stringify(m[row - 1]).replace(/\,/g, spaceArray[spaces_needed - 1])
	...
	this.import = function(data)
	{
		...
		if (self.pressing_sequence.length > 0) {self.enableSequenceButton()};
	...
	this.enableSequenceButton = function()
	{
		d3.select("#sequence").style("opacity", 1)
			.on("mouseover", self.hoverObject)
			.on("mouseout", self.offHover)
			.on("click", self.sequenceOnClick);
	}

	this.disableSequenceButton = function()
	{
	...
	this.sequenceOnClick = function()
	{
		self.overlay("S");
	}

	this.enableMatrixButton = function()
	{
		d3.select("#matrix").style("opacity", 1)
			.on("mouseover", self.hoverObject)
			.on("mouseout", self.offHover)
			.on("click", self.matrixOnClick);
	}

	this.disableMatrixButton = function()
	{
	...
	this.createUI = function(svg, y_spacing, top_offset) {
		...
		var width =340, height =60, text_Y_spacing = 42.45, x=0, sc = 1.0, rx = 25, onMouseOver = self.hoverObject;
		this.buttonAttrs = [
			{"id":"mode_mg", textID:"mode_mg_text", textColor:"black", "x":0, text_X_Spacing:116 ,text_Y_spacing: text_Y_spacing * 1, "y":0, width, height, "fill":"#999999", rx, onClick: self.changeMode, onMouseOver: self.hoverObject, onMouseOut:self.offHover, buttonText:"BUILD"},
			{"id":"back",  textID:"back_text", textColor:"black", "x":0, text_X_Spacing:40 ,text_Y_spacing : text_Y_spacing * 3, "y":1, "width":width - 190, height, "fill":"#999999",rx, onClick: self.undo, onMouseOver: self.hoverObject, onMouseOut:self.offHover, buttonText:"UNDO"},
			{"id":"forward", textID:"forward_text", textColor:"black", "x":180, text_X_Spacing:220,text_Y_spacing : text_Y_spacing * 3, "y":1, "width": width - 190, height, "fill":"#999999","rx":25, onClick: self.redo, onMouseOver: self.hoverObject, onMouseOut:self.offHover, buttonText:"REDO"},
			{"id":"color",  textID:"color_text", textColor:"white", "x":0, text_X_Spacing:115,text_Y_spacing : text_Y_spacing * 5, "y":2, width, height, "fill":"#000000", rx, onClick: self.toggleColor, onMouseOver: self.hoverObject, onMouseOut:self.offHover, buttonText:"COLOR"},
			{"id":"reset", textID:"reset_text", textColor:"black",  "x":0, text_X_Spacing:115, text_Y_spacing : text_Y_spacing * 7, "y":3, width, height, "fill":"#999999", rx, onClick: self.clear, onMouseOver, onMouseOut:self.offHover, buttonText:"RESET"},
			{"id":"random",  textID:"random_text", textColor:"black", "x":0, text_X_Spacing:105, text_Y_spacing : text_Y_spacing * 9, "y":4, width, height, "fill":"#999999", rx, onMouseOver, onMouseOut:self.offHover, onClick: function(){self.random();},buttonText:"RANDOM"},
			{"id":"store",  textID:"store_text", textColor:"black", "x":0, text_X_Spacing:115, text_Y_spacing : text_Y_spacing * 11, "y":5, width, height, "fill":"#999999",rx, onMouseOver: self.hoverObject, onMouseOut:self.hoverObject, onClick: function(){},buttonText:"STORE"},
			{"id":"recall",  textID:"recall_text", textColor:"black", "x":0,text_X_Spacing:110, text_Y_spacing : text_Y_spacing * 13, "y":6, width, height, "fill":"#999999",rx, onMouseOver: self.hoverObject, onMouseOut:self.hoverObject, onClick: function(){},buttonText:"RECALL"},
			{"id":"playback",  textID:"playback_text", textColor:"black", "x":0, text_X_Spacing:90, text_Y_spacing : text_Y_spacing * 15, "y":7, width, height, "fill":"#999999",rx, onMouseOver: self.hoverObject, onMouseOut:self.hoverObject, onClick: function(){},buttonText:"PLAYBACK"},
			{"id":"matrix",  textID:"matrix_text", textColor:"black", "x":0, text_X_Spacing:110, text_Y_spacing : text_Y_spacing * 17, "y":8, width, height, "fill":"#999999", rx, onMouseOver: self.hoverObject, onMouseOut:self.offHover, onClick: function(){self.matrixOnClick;},buttonText:"MATRIX"},
			{"id":"sequence", textID:"sequence_text", textColor:"black", "x":0, text_X_Spacing:90, text_Y_spacing : text_Y_spacing * 19,"y":9, width, height, "fill":"#999999", rx, onMouseOver:function(){}, onMouseOut:function(){}, onClick:function(){}, buttonText:"SEQUENCE"},
			{"id":"about",  textID:"about_text", textColor:"black", "x":0, text_X_Spacing:90, text_Y_spacing : text_Y_spacing * 21,"y":10, "width": width - 70, height, "fill":"#999999", rx, onMouseOver: self.hoverObject, onMouseOut:self.offHover, onClick: function(){self.overlay("A");},buttonText:"ABOUT"},
			{"id":"help",  textID:"help_text", textColor:"black", "x":275, text_X_Spacing:298, text_Y_spacing : text_Y_spacing * 21, "y":10, "width": height, height, "fill":"#0c69ff", "rx": rx + 25, "ry":50, onMouseOver: self.hoverObject, onMouseOut:self.offHover, onClick: function(){self.overlay("H");},buttonText:"?" }
		];
		...
		ui.append("g")
			...
			.each(function(buttonAttrs, i) { // adds button texts
				...
				append_text(selection, buttonText, textID, buttonAttrs['text_X_Spacing'], buttonAttrs['y']*y_spacing+text_Y_spacing+top_offset, buttonAttrs["textColor"], 3);
			});
			...
			margin.append("g")
				.attr("id", "sponsored_g");
			margin.append("g").attr("id", "santander_logo_g")
